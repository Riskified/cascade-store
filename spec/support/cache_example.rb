RSpec.shared_examples 'a cascading cache' do |cache, store1, store2|
  before(:each) do
    cache.clear
  end
  ...
  xit 'test_crazy_key_characters' do
    crazy_key = "#/:*(<+=> )&$%@?;'\"\'`~-"
  ...
  it 'test_empty_store_cache_miss' do
    empty_cache = ActiveSupport::Cache.lookup_store(:cascade_store)
    expect(empty_cache.write('foo', 'bar')).to be true
    expect(empty_cache.fetch('foo')).to be nil
  end
  ...
  xit 'test_cascade_increment_partial_returns_num' do
    store2.write('foo', 0)
    expect(cache.increment('foo', 1)).to eq 1
    expect(cache.read('foo')).to eq 1
  end

  xit 'test_cascade_decrement_partial_returns_num' do
    store2.write('foo', 1)
